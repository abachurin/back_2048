...
# separate pieces of the board, and that obviously can not lead to anything.
# but 2) we don't want them to intersect too much (like 3 cells common to two quartets), as they start
# to kind of contradict each other.
...
#    1114112 (=65536 * 17) features and dot-product it by the vector of 1114122 weights (case n=4).
...
# 4) The same goes for back-propagation. We only need to update 17 numbers out of 1m+ on every step.
# 5) Of course, we update 17 * 8 weights using an obvious D4 symmetry group acting on the board
...
class QAgent:
    ...
    def __init__(self, config: dict):
        ...
        self.low_alpha_limit = 0.01
        self.weights = None
        self.weight_signature = None

        # get parameters from config, leaves default if not provided
        for key in config:
        ...
        self.stop_file = f'stop_{self.name}.json'
        self.print = functools.partial(S3.add_log, user=self.user) if self.web else print
    ...
    def save_agent(self):
        self.list_to_np()
        if self.s3:
            S3.save(self, self.file, 'agents')
        else:
            with open(self.file, 'wb') as f:
                pickle.dump(self, f, -1)
        self.np_to_list()

    def save_best_game(self, game):
        if self.s3:
            S3.save(game, self.game_file, 'games')
        else:
            game.save_game(self.game_file)

    def load_agent_local(self, name):
        self.print('loading agent ...')
        with open(f'{name}.pkl', 'r') as f:
            agent = pickle.load(f)
        agent.np_to_list()
        return agent
    ...
    def load_agent(self, name):
        self.print('loading agent ...')
        agent = S3.load(f'{name}.pkl', 'agents')
        agent.np_to_list()
        return agent
    ...
    def train_run(self, num_eps=100000):
        av1000, ma100 = [], deque(maxlen=100)
        reached = [0] * 7
        best_of_1000 = Game()
        global_start = start = time.time()
        self.print(f'{clock()}: Agent {self.name} training session started, current step = {self.step}')
        self.print(f'Agent will be saved every 1000 episodes and on STOP command')
        for i in range(self.step + 1, self.step + num_eps + 2):
            # check if train run stopped by User
            stopper = S3.load(self.stop_file, 'stop') or {}
            if stopper.get('stop', False):
                S3.delete(self.stop_file, 'stop')
                break
            ...
            if i % 100 == 0:
                ...
                self.print(f'{clock()}: episode {i}, score {game.score} reached {1 << max_tile} ma_100 = {ma}')
            if i % 1000 == 0:
                ...
                self.print(f'best game of this Agent:')
                self.print(self.top_game.__str__())
                self._display_lr()
                self.print('------\n')
                self.save_agent()
                self.print(f'{self.name} saved')
                best_of_1000 = Game()
        total_time = int(time.time() - global_start)
        self.print(f'{clock()}: Total time = {total_time // 60} min {total_time % 60} sec')
        self.save_agent()
